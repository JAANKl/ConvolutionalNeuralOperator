...
def plot_samples(data_loader, model, n, device, cmap='coolwarm', vmin=None, vmax=None, autoreg=True):
    model.eval()
    with torch.no_grad():
        for i, batch in enumerate(data_loader):
            for inputs, outputs in batch:
                if i == n:
                    ...
                    plt.tight_layout()
                    plt.show()
                    plt.savefig('FNO_output_plot.png')
                    break
...
plot_samples(test_loader, model, n=10, device=device, cmap='coolwarm', vmin=None, vmax=None, autoreg=True)
